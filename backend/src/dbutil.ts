...
import {
  HeatPumpWaterHeaterQuery,
  HeatPumpDryerQuery,
  HeatPumpHvacQuery,
} from "./model/appliance_query";
...
export const RAW_WATER_HEATERS = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../data/water-heaters.json"), "utf-8")
);
export const RAW_DRYERS = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../data/dryers.json"), "utf-8")
);
export const RAW_HEATPUMPS = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../data/hvac.json"), "utf-8")
);
export const RAW_STOVE = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../data/stove.json"), "utf-8")
);

const PROCESSED_WATER_HEATERS = RAW_WATER_HEATERS.map((d) =>
  toQueryable(d)
) as dt.HeatPumpWaterHeater[];
const PROCESSED_DRYERS = RAW_DRYERS.map((d) =>
  toQueryable(d)
) as dt.HeatPumpDryer[];
const PROCESSED_HEATPUMPS = RAW_HEATPUMPS.map((d) =>
  toQueryable(d)
) as dt.HeatPump[];
...
function toQueryable<Type extends dt.Appliance>(data: Type): Type {
  data.weight.value = convertUnit(
    data.weight.value,
    data.weight.unit,
    DbDefaultUnits["weight"]
  );
  data.weight.unit = DbDefaultUnits["weight"];
  return data;
}

function toViewUnits<Type extends dt.Appliance>(
  d: Type,
  dimensionUnit: string,
  weightUnit: string
): Type {
  d.dimensions.length = convertUnit(
    d.dimensions.length,
    d.dimensions.unit,
    dimensionUnit
  );
  d.dimensions.width = convertUnit(
    d.dimensions.width,
    d.dimensions.unit,
    dimensionUnit
  );
  d.dimensions.height = convertUnit(
    d.dimensions.height,
    d.dimensions.unit,
    dimensionUnit
  );
...
export function queryWaterHeater(q: HeatPumpWaterHeaterQuery) {
  return PROCESSED_WATER_HEATERS.filter(
    (heater) =>
...
export function queryDryer(q: HeatPumpDryerQuery) {
  return PROCESSED_DRYERS.filter(
    (dryer) =>
...
export function queryHvac(q: HeatPumpHvacQuery) {
  return PROCESSED_HEATPUMPS.filter(
    (hp) =>
